...
**Referans aktivasyon değeri (şablon derleme).** Depodaki blob çözüldüğünde düz metin **`123456`** üretilir. Arayüz yalnızca **rakam** kabul eder, girişi **en fazla 6** haneyle sınırlar; **`trim`** sonrası girdi ile **`expectedCode`** arasında **tam eşleşme** ve her iki taraf için **uzunluk 6** sağlanmadan **`ActivationCodeScreen`** içindeki **`onActivationSuccess`** tetiklenmez — aksi durumda **`activation_invalid_code`** gösterilir ve **`is_activated`** **false** kalır (ör. **Ayarlar**’daki konum sıklığı bölümü kilitli kalır). **`123456`** yalnızca **yerel demo** içindir; üretim öncesi **`CIPHER_TEXT_B64`**’ü yeniden üretin ve cihazda saklı Keystore kopyasını da politikanıza göre sıfırlayın.
