...
https://github.com/user-attachments/assets/83e3f8c4-526c-407a-8b54-01d304adb73b
...
**Reference activation value (template build).** The field accepts **digits only**, **six characters maximum**, and **`submitActivation`** requires **`trim()` equality** between input and **`expectedCode`**, plus **`length == 6`** for both sides, before **`onActivationSuccess`** runs; otherwise **`activation_invalid_code`** is shown and **`is_activated`** remains **false**, which keeps dependent gates (e.g. **Settings** location-interval chips) locked. **The demo activation code in this template is `123456`**—local testing only. Before production, rotate the embedded payload (`EmbeddedActivationPayload` / **`CIPHER_TEXT_B64`**).
