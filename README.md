...
**Activation code (this repository build):** **`123456`** — enter exactly these six digits on the onboarding activation screen. This is a **demo default** only.
...
**Activation entry.** **`ActivationCodeScreen`** compares user input against **`AppActivationStore.getExpectedActivationCode()`**. That expectation is produced locally from embedded AES-GCM material (**`EmbeddedActivationPayload`** under **`app/.../activation`**), then optionally **re-sealed** with **hardware Keystore-backed** AES-GCM values in **`activation_prefs` DataStore**—this reference build performs **no** remote OTP issuance.

**Reference activation value (template build).** The checked-in blob decrypts to **`123456`**. The field accepts **digits only**, **six characters maximum**, and **`submitActivation`** requires **`trim()` equality** between input and **`expectedCode`**, plus **`length == 6`** for both sides, before **`onActivationSuccess`** runs; otherwise **`activation_invalid_code`** is shown and **`is_activated`** remains **false**, which keeps dependent gates (e.g. **Settings** location-interval chips) locked. Treat **`123456`** as a **demo default**—regenerate **`CIPHER_TEXT_B64`** (and clear any stored Keystore copy) before shipping a production credential policy.
